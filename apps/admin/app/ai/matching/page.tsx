...
export default function MatchingMonitorPage() {
  ...
  if (err) return <div className="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-600">{err}</div>;
  if (!data) return <div className="text-sm text-ink-500">加载中…</div>;

  const c = data.matchCalls;
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
        <Kpi label="匹配调用(30d)" value={c.total} />
        <Kpi label="LLM 语义命中率" value={`${c.llmRate}%`} sub={`LLM ${c.llm} · 降级 ${c.rule} · 空 ${c.empty}`} />
        <Kpi
          label="技师 persona 覆盖"
