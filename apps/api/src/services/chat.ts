...
export async function sendMessage(
  ...
  args: {
    ...
    type?: 'text' | 'image' | 'voice' | 'media_locked' | 'order_offer' | 'schedule_offer' | 'gift_hint';
    mediaRef?: string;
    isAiAlter?: boolean;
    isEncrypted?: boolean;
  },
  ...
  // order_offer / schedule_offer / gift_hint 的 text 是结构化 JSON(卡片 offer),非自然语言,跳过红线检查(同 image/voice)
  if (!args.isEncrypted && args.type !== 'image' && args.type !== 'voice' && args.type !== 'order_offer' && args.type !== 'schedule_offer' && args.type !== 'gift_hint') {
    try {
  ...
  // order_offer / schedule_offer / gift_hint 是 JSON 卡片,翻译无意义 → 跳过省 LLM 成本
  if (!args.isEncrypted && args.type !== 'order_offer' && args.type !== 'schedule_offer' && args.type !== 'gift_hint') {
    fireAndForget(
