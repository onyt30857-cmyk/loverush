...
export async function sendMessage(
  ...
  args: {
    ...
    type?: 'text' | 'image' | 'voice' | 'media_locked' | 'order_offer' | 'schedule_offer' | 'gift_hint' | 'chat_paywall';
    mediaRef?: string;
    isAiAlter?: boolean;
    isEncrypted?: boolean;
  },
  ...
  if (!args.isEncrypted && args.type !== 'image' && args.type !== 'voice' && args.type !== 'order_offer' && args.type !== 'schedule_offer' && args.type !== 'gift_hint' && args.type !== 'chat_paywall') {
    try {
  ...
  // order_offer / schedule_offer / gift_hint / chat_paywall 是 JSON 卡片,翻译无意义 → 跳过省 LLM 成本
  if (!args.isEncrypted && args.type !== 'order_offer' && args.type !== 'schedule_offer' && args.type !== 'gift_hint' && args.type !== 'chat_paywall') {
    fireAndForget(
